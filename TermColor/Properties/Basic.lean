...
private
theorem foldl_append_left
    (init : String)
    (texts : List String)
    : texts.foldl (· ++ ·) init = init ++ texts.foldl (· ++ ·) ""
    := by
...
theorem join_append
    ...
    : String.join (left ++ right) = String.join left ++ String.join right
    := by
  simp [String.join, List.foldl_append, foldl_append_left (left.foldl (· ++ ·) "")]

theorem render_plain
    (text : Text)
    : Text.render .plain text = text.plainText
    := by
  simp [Text.render, Text.plainText, Text.renderSegment_plain]

theorem concat_append
    (left right : List Text)
    : Text.concat (left ++ right) = Text.concat left ++ Text.concat right
    := by
  change _ = Text.append _ _
  simp [Text.concat, Text.append, List.flatMap_append]

theorem render_append
    (target : RenderTarget)
    (left right : Text)
    : Text.render target (left ++ right) = Text.render target left ++ Text.render target right
    := by
